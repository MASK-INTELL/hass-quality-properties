...
export async function middleware(request: NextRequest) {
  ...
  response.headers.set('Content-Security-Policy', [
    ...
    `img-src 'self' data: blob: https://*.r2.dev https://images.unsplash.com https://*.supabase.co https://cdn.hassproperties.online`,
    `font-src 'self' data:`,
    `object-src 'none'`,
    `base-uri 'self'`,
    `frame-src 'self' https://vercel.live https://*.supabase.co https://*.r2.dev`,
    `media-src 'self' https://*.r2.dev https://cdn.hassproperties.online`,
    `frame-ancestors 'none'`,
    `connect-src 'self' https://*.supabase.co wss://*.supabase.co https://*.r2.cloudflarestorage.com https://cdn.hassproperties.online https://*.google-analytics.com`,
    `manifest-src 'self'`,
