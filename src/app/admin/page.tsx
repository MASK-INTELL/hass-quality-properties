...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Listings', value: stats.totalProperties, icon: Building2, color: 'emerald', sub: 'All active listings' },
    { label: 'For Sale', value: stats.forSale, icon: Home, color: 'blue', sub: 'Properties on market' },
    { label: 'For Rent', value: stats.forRent, icon: TrendingUp, color: 'purple', sub: 'Rental listings' },
    { label: 'Vehicles', value: stats.vehicles, icon: Car, color: 'amber', sub: 'Cars & motorcycles' },
  ];
            ...
            {recentProperties.map((property) => (
              <div key={property.id} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  <Link href={`/admin/properties/edit/${property.id}`} className="text-xs text-emerald-600 hover:text-emerald-700 font-medium">
                    Edit
                  </Link>
                </div>
              </div>
